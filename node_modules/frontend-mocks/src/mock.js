...
const FILENAME_LENGTH_LIMIT = 255;
...
class Mock {
  constructor(options = {}) {
    ...
    this.options = {
      ...
      // For historical reasons the default value of `hashDirname` is true, would be nice to change it to false
      // in the feature with some another breaking change
      hashDirname: typeof options.hashDirname === 'undefined' ? true : Boolean(options.hashDirname),
    };
  ...
  writeFile(name, content) { // eslint-disable-line class-methods-use-this
    const fileContent = typeof content === 'string' ? content : JSON.stringify(content, null, 2);

    return new Promise((resolve, reject) => {
      mkdirp(path.dirname(name)).then(() => {
  ...
  mockFilename(options, transformers = {}) {
    ...
    if (transformers.includeHeaders) {
      if (Array.isArray(transformers.includeHeaders)) {
        transformers.includeHeaders.forEach((headerName) => {
          if (options.headers && Object.prototype.hasOwnProperty.call(options.headers, headerName)) {
            params[headerName] = options.headers[headerName];
          }
        });
      } else {
        log.warn('transformers.includeHeaders must be an array');
      }
    }
    ...
    if (options.body) {
      ...
      if (body) {
        ...
        if (typeof body === 'object') {
          const obj = omit({ ...body }, transformers.ignoreData);
          ...
          qs = qs.length > 0 ? `${qs}&${queryString.stringify(obj, {
            skipEmptyString: true,
            skipNull: true,
    ...
    if (filename.length > FILENAME_LENGTH_LIMIT - '.json'.length) {
      if (this.options.hashDirname) {
        filename = crypto.createHash('md5').update(filename).digest('hex');
      } else {
        const { dir, base } = path.parse(filename);
        if (base.length > FILENAME_LENGTH_LIMIT - '.json'.length) {
          filename = path.join(dir, crypto.createHash('md5').update(base).digest('hex'));
        }
      }
    }
  ...
  _patchRequest() {
    ...
    const getPatchedRequest = (proto = 'http') => {
      const defaultAgent = proto === 'https' ? https.globalAgent : http.globalAgent;
      ...
      return (...args) => {
        ...
        const callback = typeof args[0] === 'function' ? args[0] : function noop() {
        };
        ...
        opts._defaultAgent = defaultAgent;

        if (!opts.protocol) {
          opts.protocol = opts.agent && opts.agent.protocol ? opts.agent.protocol : opts._defaultAgent.protocol;
        }
          ...
          this.getMockedData(opts, result.transformers)
            .then((responseData) => {
              ...
              mockData.push(JSON.stringify(typeof result.transformers.transformResponse === 'function'
                ? result.transformers.transformResponse(responseData.data, req) : responseData.data));
              ...
              if (typeof result.transformers.transformHeaders === 'function') {
                responseData.headers = result.transformers.transformHeaders(responseData.headers, req);
              }
              ...
              const ondata = function ondata() {
                ...
                if (!req.aborted) {
                  if (typeof chunk === 'string') {
                    chunk = Buffer.from(chunk, res.charset || 'utf8');
                  }
                  if (res.charset) {
    ...
    https.request = getPatchedRequest('https');
    http.request = getPatchedRequest('http');
  }
  ...
  getMockedData(options, transformers) {
    ...
    Please review the content of ${friendlyFilename}`);
          ...
          return this.writeFile(mockFilename, data);
        })
  ...
  getOriginalData(options) { // eslint-disable-line
    ...
    return new Promise((resolve) => {
      ...
      const makeRequest = (isSecure ? originalHTTPSRequest : originalHTTPRequest);

      const req = makeRequest(options, res => this.handleOriginalDataRequest(resolve, res));
  ...
  handleOriginalDataRequest(resolve, res) {
    const body = [];
    res.on('data', (chunk) => {
      body.push(chunk);
    });

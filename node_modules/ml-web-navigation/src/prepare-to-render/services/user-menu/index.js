...
const { DEVELOPMENT, PRODUCTION, SCOPE } = require('frontend-env');
const { isPlus, isLite } = require('../../../layout-type-detection');
...
const execute = ({ req, siteId, user, type }) => {
  ...
  } else if (DEVELOPMENT && req.cookies.meliLab) {
    payload.params.env = req.cookies.meliLab.startsWith('test')
      ? req.cookies.meliLab
      : `test${req.cookies.meliLab}`;
  }

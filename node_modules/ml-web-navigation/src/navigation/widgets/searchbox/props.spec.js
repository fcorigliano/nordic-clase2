...
describe('Mercado Libre Navigation', () => {
  describe('Searchbox Props', () => {
    it('returns the necessary props for the searchbox widget', () => {
      const props = searchboxProps(baseProps);

      expect(props.platformId).toEqual(baseProps.platform.id);
      expect(props.siteId).toEqual(baseProps.platform.siteId);
      expect(props.searchEndpoint).toEqual(`${baseProps.urls.listing.link}/$query`);
      expect(props.categoryEndpoint).toEqual(null);
      expect(props.isUserLoggedIn).toEqual(false);
      expect(props.siteDomain).toEqual(baseProps.urls.mainDomain.link);
      expect(props.deviceType).toEqual(baseProps.device.type);
      expect(props.baseUrl).toEqual(baseProps.cdn.link);
      expect(props.version).toEqual(baseProps.navigationFeatures.searchBoxVersion);
    });

    it('returns the necessary props for the searchbox widget with a category, but since there is no permalink, categoryEndpoint is still null', () => {
      const categoryProps = { ...baseProps, category: {} };
      const props = searchboxProps(categoryProps);
    ...
    it('returns the necessary props for the searchbox widget with a category', () => {
      const categoryProps = { ...baseProps,
        category: {
          permalink: 'https://celulares.mercadolibre.com.ar',
        } };
      const props = searchboxProps(categoryProps);
      ...
      expect(props.siteId).toEqual(baseProps.platform.siteId);
      expect(props.searchEndpoint).toEqual(`${baseProps.urls.listing.link}/$query`);
      expect(props.categoryEndpoint).toEqual(categoryProps.category.permalink);
      expect(props.isUserLoggedIn).toEqual(false);
      expect(props.siteDomain).toEqual(baseProps.urls.mainDomain.link);
      expect(props.deviceType).toEqual(baseProps.device.type);
      expect(props.baseUrl).toEqual(baseProps.cdn.link);

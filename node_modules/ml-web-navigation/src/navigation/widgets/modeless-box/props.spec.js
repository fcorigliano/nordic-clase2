const modelessBoxProps = require('./props');
...
describe('Mercado Libre Navigation', () => {
  describe('Modeless box Props', () => {
    it('returns the necessary props for the modeless box widget', () => {
      const props = modelessBoxProps(baseProps);

      expect(props.version).toEqual(undefined);
      expect(props.baseUrl).toEqual(baseProps.cdn.link);
      expect(props.mainDomain).toEqual(baseProps.urls.mainDomain.link);
    });

    it('returns the necessary props for the modeless box widget with existing modeless box version', () => {
      const props = modelessBoxProps({
        ...baseProps,
        navigationFeatures: {
          modelessBoxVersion: '1.0.0',
        },
      });

      expect(props.version).toEqual('1.0.0');
      expect(props.baseUrl).toEqual(baseProps.cdn.link);
      expect(props.mainDomain).toEqual(baseProps.urls.mainDomain.link);
    });
  });

const onboardingProps = require('./props');
...
defaultProps.navigationUrls = defaultProps.urls;

describe('Mercado Libre Navigation', () => {
  describe('Onboarding cp Props', () => {
    it('returns the necessary props for the onboarding cp widget', () => {
      const baseProps = { ...defaultProps, currentLocation: { showOnboarding: true } };

      const props = onboardingProps(baseProps);

      expect(props.show).toEqual(true);
      expect(props.siteId).toEqual(baseProps.platform.siteId);
      expect(props.domain).toEqual(baseProps.platform.domain);
      expect(props.baseUrl).toEqual(baseProps.cdn.link);
      expect(props.locale).toEqual('es_AR');
      expect(props.version).toEqual(baseProps.navigationFeatures.onBoardingCPVersion);
    });

    it('show should be false if type is not plus', () => {
      const baseProps = { ...defaultProps, currentLocation: { showOnboarding: true }, type: 'some-type' };

      const { show } = onboardingProps(baseProps);

const searchboxProps = require('./props');
...
describe('Mercado Libre Navigation', () => {
  describe('Searchbox props', () => {
    it('returns props for searchbox', () => {
      const props = searchboxProps(baseProps);

      expect(props.defaultValue).toEqual(baseProps.searchValue);
      expect(props.isFocus).toEqual(baseProps.searchFocus);
      expect(props.closeTitle).toEqual('Cerrar');
      expect(props.clearTitle).toEqual('Limpiar');
      expect(props.isLowEnd).toEqual(baseProps.device.lowEnd);
      expect(props.category).toEqual(baseProps.category);
      expect(props.action).toEqual(`https://www.${baseProps.platform.domain}/jm/search`);
      expect(props.inputLabel).toEqual(baseProps.configSchema.i18n.searchInputLabel);
      expect(props.placeholder).toEqual(baseProps.configSchema.i18n.searchPlaceholder);
      expect(props.iconLabel).toEqual(baseProps.configSchema.i18n.searchIconLabel);
      expect(props.placeholderMobile).toEqual(baseProps.configSchema.i18n.searchPlaceholderMobile);
    });

    it('returns props for searchbox with close title and clear title coming from props', () => {
      const labeli18n = { ...baseProps.configSchema.i18n,
        closeTitle: 'Close that comes from props',
        clearTitle: 'Clear that comes from props' };
      const labelProps = { ...baseProps,
        configSchema: { ...baseProps.configSchema, i18n: labeli18n } };
      const props = searchboxProps(labelProps);

      expect(props.isFocus).toEqual(baseProps.searchFocus);
      expect(props.closeTitle).toEqual('Close that comes from props');
      expect(props.clearTitle).toEqual('Clear that comes from props');
      expect(props.isLowEnd).toEqual(baseProps.device.lowEnd);
      expect(props.category).toEqual(baseProps.category);
      expect(props.action).toEqual(`https://www.${baseProps.platform.domain}/jm/search`);
      expect(props.inputLabel).toEqual(baseProps.configSchema.i18n.searchInputLabel);

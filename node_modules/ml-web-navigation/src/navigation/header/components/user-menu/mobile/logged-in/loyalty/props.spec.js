const loyaltyProps = require('./props');
...
describe('Mercado Libre Navigation', () => {
  describe('Loyalty props', () => {
    it('returns props needed for the Loyalty Component', () => {
      const newProps = { ...baseProps,
        user: { nickname: 'test' },
        userInfo: { title: 'Hola test!', loyal: { level: 1, percentage: 10 } } };
      const props = loyaltyProps(newProps);

      expect(props.href).toEqual(baseProps.urls.loyalty.link);
      expect(props.level).toEqual(newProps.userInfo.loyal.level);
      expect(props.percentage).toEqual(newProps.userInfo.loyal.percentage);
      expect(props.levelName).toEqual(newProps.userInfo.loyal.full_level_name);
    });
  });
});

const userLinksSectionsProps = require('./props');
...
describe('Mercado Libre Navigation', () => {
  describe('User Links Sections', () => {
    it('has the expected enhancements for the guest type', () => {
      const props = userLinksSectionsProps(baseProps);

      expect(props.sectionsLinks).toBeDefined();
      expect(Array.isArray(props.sectionsLinks)).toEqual(true);
      expect(Array.isArray(props.sectionsLinks[0])).toEqual(true);

      props.sectionsLinks[0].forEach((link) => {
        expect(link.link.href.endsWith('menu-user')).toEqual(true);
      });
    });

    it('has the expected enhancements for the logged type', () => {
      const props = userLinksSectionsProps(baseProps, 'logged');

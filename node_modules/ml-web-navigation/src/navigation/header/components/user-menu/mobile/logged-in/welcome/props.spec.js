...
const newProps = { ...baseProps,
  ...
  userInfo: { title: 'Hola test!' } };

describe('Mercado Libre Navigation', () => {
  describe('Logged in User Welcome props', () => {
    it('returns props for welcome component without loyalty in mobile', () => {
      const welcomeProps = props(newProps);

      expect(welcomeProps.firstName).toEqual(newProps.navigationPIIData.preferredName);
      expect(welcomeProps.nickname).toEqual(newProps.user.nickname);
      expect(welcomeProps.title).toEqual(newProps.userInfo.title);
      expect(welcomeProps.email).toEqual(newProps.navigationPIIData.email);
      expect(welcomeProps.hasLoyalty).toBeFalsy();
      expect(welcomeProps.loyalty).toEqual({});
      expect(welcomeProps.domain).toEqual(baseProps.urls.mainDomain.link);
      expect(welcomeProps.avatar).toBeDefined();
    });

    it('returns props for welcome component without loyalty in desktop', () => {
      const propsInDesktop = { ...newProps, device: { mobile: false, desktop: true } };

      const welcomeProps = props(propsInDesktop);
    ...
    it('returns props for welcome component with empty loyalty', () => {
      const propsWithoutLoyalty = { ...newProps, userInfo: { title: newProps.userInfo.title } };

      const welcomeProps = props(propsWithoutLoyalty);
    ...
    it('returns props for welcome component with loyalty', () => {
      const propsWithLoyalty = { ...newProps, userInfo: { title: 'Hola test!', loyal: { level: 1, percentage: 10, full_level_name: 'test' } } };

      const welcomeProps = props(propsWithLoyalty);
      ...
      expect(welcomeProps.email).toEqual(newProps.navigationPIIData.email);
      expect(welcomeProps.loyalty).toEqual({
        href: 'https://www.mercadolibre.com.ar/puntos',
        level: propsWithLoyalty.userInfo.loyal.level,
        levelName: propsWithLoyalty.userInfo.loyal.full_level_name,
        percentage: propsWithLoyalty.userInfo.loyal.percentage,
      });
      expect(welcomeProps.hasLoyalty).toBeTruthy();
    });
  });
});

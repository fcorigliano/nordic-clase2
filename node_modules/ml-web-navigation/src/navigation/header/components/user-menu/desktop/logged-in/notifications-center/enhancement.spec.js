...
const notificationsEnhancement = require('./enhancement');
...
describe('Mercado Libre Navigation', () => {
  describe('Notifications Enhancement', () => {
    it('returns a valid set of props for notifications link', () => {
      const url = {
        link: {
          href: 'https://www.mercadolibre.com.ar',
          className: 'option-notifications',
        },
        name: 'Notificaciones',
      };

      const urlWithInjectedProps = notificationsEnhancement.enhance(url, baseProps);

      expect(urlWithInjectedProps.link.href).toEqual(url.link.href);
      expect(urlWithInjectedProps.name).not.toEqual(url.name);
      expect(urlWithInjectedProps.name).toBeDefined();
      expect(urlWithInjectedProps.link.className).toEqual('option-notifications notifications-widget');
    });

    it('returns matches the props snapshot', () => {
      const url = {
        link: {
          href: 'https://www.mercadolibre.com.ar',
          className: 'option-notifications',
      ...
      expect(urlWithInjectedProps).toMatchSnapshot();
    });

    it('returns a valid set of props for notifications link for desktop', () => {
      const url = {
        link: {
      ...
      const urlWithInjectedProps = notificationsEnhancement.enhance(url, { ...baseProps, device: { mobile: false, desktop: true } });

      expect(urlWithInjectedProps.link.href).toEqual('https://www.mercadolibre.com.ar/gz/notifications/center#nav-header');
      expect(urlWithInjectedProps.name).not.toEqual(url.name);
      expect(urlWithInjectedProps.name).toBeDefined();
      expect(urlWithInjectedProps.link.className).toEqual('option-notifications notifications-widget');
    });

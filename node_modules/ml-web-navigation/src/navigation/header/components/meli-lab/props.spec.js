const meliLabProps = require('./props');
...
describe('Mercado Libre Navigation', () => {
  describe('Meli Lab props', () => {
    it('returns props when meli lab is not enabled', () => {
      const props = meliLabProps(baseProps);

      expect(props.labUrl).toEqual(`${baseProps.urls.baseDomain.link}/gz/labs`);
      expect(props.isEnabled).toBeDefined();
      expect(props.isEnabled).toEqual(false);
      expect(props.melilab).not.toBeDefined();
      expect(props.isMobile).toEqual(true);
    });

    it('returns props when meli lab is enabled', () => {
      const melilabeEnabledProps = { ...baseProps, melilab: 'z' };
      const props = meliLabProps(melilabeEnabledProps);

      expect(props.labUrl).toEqual(`${baseProps.urls.baseDomain.link}/gz/labs`);
      expect(props.isEnabled).toBeDefined();
      expect(props.isEnabled).toEqual(true);
      expect(props.melilab).toEqual('z');
      expect(props.isMobile).toEqual(true);
    });
  });
});

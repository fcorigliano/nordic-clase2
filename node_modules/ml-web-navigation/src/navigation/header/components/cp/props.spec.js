...
describe('Mercado Libre Navigation', () => {
  describe('CP Props test', () => {
    it('should match base props with context, mobile, cpInfo and showing', () => {
      const result = CPProps(baseProps);

      expect(result.linkCP.href).toBe('https://www.mercadolibre.com.ar/navigation/addresses-hub?go=https%3A%2F%2Fwww.mercadolibre.com.ar%2F&context=cart');
      expect(result.linkCP.className).toBe('nav-header-cp-anchor');
      expect(result.label).toBe('Enviar a Capital Federal');
      expect(result.title).toBe('Enviar a');
      expect(result.isDesktop).toBe(false);
      expect(result.show).toBe(true);
    });

    it('should match base props with context, mobile, cpInfo and not show', () => {
    ...
    it('should match base props with no context, mobile, cpInfo and showed', () => {
      const props = { ...baseProps,
        features: {
          menuShippingLocation: {
            shippingMobileContext: null,
            isShippingLocation: true,
          },
        } };
      const result = CPProps(props);

      expect(result.linkCP.href).toBe('https://www.mercadolibre.com.ar/navigation/addresses-hub?go=https%3A%2F%2Fwww.mercadolibre.com.ar%2F');
      expect(result.linkCP.className).toBe('nav-header-cp-anchor');
      expect(result.label).toBe('Enviar a Capital Federal');
      expect(result.title).toBe('Enviar a');
      expect(result.isDesktop).toBe(false);
    ...
    it('should match base props with context, mobile, no cpInfo and showed', () => {
      const props = { ...baseProps, cpInfo: null };
      const result = CPProps(props);

      expect(result.linkCP.href).toBe('https://www.mercadolibre.com.ar/navigation/addresses-hub?go=https%3A%2F%2Fwww.mercadolibre.com.ar%2F&context=cart');
      expect(result.linkCP.className).toBe('nav-header-cp-anchor');
      expect(result.label).toBe('Enviar a');
      expect(result.isDesktop).toBe(false);
      expect(result.show).toBe(true);
    });

    it('should match base props with context, not mobile, cpInfo and showed', () => {
      const props = { ...baseProps, device: { mobile: false, desktop: true } };
      const result = CPProps(props);

      expect(result.linkCP.href).toBe('https://www.mercadolibre.com.ar/navigation/addresses-hub?go=https%3A%2F%2Fwww.mercadolibre.com.ar%2F&context=cart');
      expect(result.linkCP.className).toBe('nav-menu-cp nav-menu-cp-logged');
      expect(result.label).toBe('Enviar a Capital Federal');
    ...
    it('should match base props with context, mobile, cpInfo, not cp title and showed', () => {
      const props = { ...baseProps, cpInfo: { title: null, label: 'Enviar a Capital Federal' } };
      const result = CPProps(props);
    ...
    it('should match base props with context, not mobile, cpInfo, not cp title and showed', () => {
      const props = { ...baseProps,
        cpInfo: { title: null, label: 'Enviar a Capital Federal' },
        device: { mobile: false, desktop: true } };
      const result = CPProps(props);

      expect(result.linkCP.href).toBe('https://www.mercadolibre.com.ar/navigation/addresses-hub?go=https%3A%2F%2Fwww.mercadolibre.com.ar%2F&context=cart');
      expect(result.linkCP.className).toBe('nav-menu-cp');
      expect(result.label).toBe('Enviar a Capital Federal');
      expect(result.title).toBe(null);
      expect(result.isDesktop).toBe(true);
      expect(result.show).toBe(true);

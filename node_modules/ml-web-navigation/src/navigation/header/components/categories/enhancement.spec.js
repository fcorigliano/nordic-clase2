...
const categoriesEnhancement = require('./enhancement');
...
describe('Mercado Libre Navigation', () => {
  describe('Categories Enhancement', () => {
    it('returns a valid set of props for categories link with isCategory', () => {
      const url = {
        link: {
      ...
      const urlWithInjectedProps = categoriesEnhancement.enhance(url, baseProps, {
        isCategory: true,
      });
      ...
      expect(urlWithInjectedProps.link.className).toEqual(`${url.className} nav-menu-categories-link`);
    });

    it('returns a valid set of props for categories link without isCategory', () => {
      const url = {
        link: {
          href: 'https://www.mercadolibre.com.ar',
        },
      ...
      expect(urlWithInjectedProps.link.href).toEqual(url.link.href);
      expect(urlWithInjectedProps.name).toEqual(url.name);
      expect(urlWithInjectedProps.SideEffect).toBeNull();
      expect(urlWithInjectedProps.link.className).toEqual(url.className);
    });
  });
});

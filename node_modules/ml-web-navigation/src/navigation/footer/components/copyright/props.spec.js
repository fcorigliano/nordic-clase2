...
describe('Mercado Libre Navigation', () => {
  describe('Copyright props', () => {
    it('returns props needed for the Copyright Component', () => {
      const companyName = 'Mercado Libre';
      const copyrightSince = '1999';
      ...
      const copyrightProps = require('./props');

      const props = copyrightProps(baseProps);
      const date = new Date();

      expect(props.since).toEqual(copyrightSince);
      expect(props.companyName).toEqual(companyName);
      ...
      expect(props.currentYear).toEqual(date.getFullYear());
    });
  });
});

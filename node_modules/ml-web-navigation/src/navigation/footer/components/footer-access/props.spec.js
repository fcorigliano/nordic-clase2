...
const mockConfiguration = (toReturn) => {
...
const mockMapLinksToUrls = (toReturn) => {
  const mapLinksToUrls = jest.fn().mockReturnValue(toReturn);
  jest.doMock('../../../utils/links', () => ({ mapLinksToUrls }));

  const assertMapLinksToUrlsCalled = (parameter) => {
    expect(mapLinksToUrls).toHaveBeenCalledWith(parameter);
  };

  return {
    assertMapLinksToUrlsCalled,
  };
};
...
describe('Mercado Libre Navigation', () => {
  ...
  describe('Footer Access props', () => {
    it('returns props needed for the Footer Access Component', () => {
      const mockedValues = {
        hidden: false,
        title: 'Mas información',
        items: [
      ...
      const { assertConfigGetCalled } = mockConfiguration(mockedValues);

      const links = [{}];
      mockMapLinksToUrls(links);

      const footerAccessProps = getModule();

      const props = footerAccessProps(baseProps);

      expect(props.title).toBe(mockedValues.title);
      props.groups.forEach((group, index) => {
        expect(group.id).toBe(mockedValues.items[index].id);
        expect(group.title).toBe(mockedValues.items[index].title);
        expect(group.links).toBe(links);
      });
      assertConfigGetCalled();
    });

    it('returns props needed for the Footer Access Component, but it returns an empty groups prop since there is no footer access in config schema', () => {
      const { assertConfigGetCalled } = mockConfiguration(null);
      const footerAccessProps = getModule();
      const props = footerAccessProps(baseProps);

      expect(props.groups).toBeDefined();
      expect(props.groups).toHaveLength(0);
      expect(props.title).not.toBeDefined();
      assertConfigGetCalled();
    });

    it('returns props needed for the Footer Access Component, but it returns an empty groups prop since there is footer access in config schema, but with no items', () => {
      const { assertConfigGetCalled } = mockConfiguration({});
      const footerAccessProps = getModule();
      const props = footerAccessProps(baseProps);
    ...
    it('returns props needed for the Footer Access Component, but no groups since the only item in the list does not have any links', () => {
      const { assertConfigGetCalled } = mockConfiguration({
        hidden: false,
        title: 'Mas información',
        items: [

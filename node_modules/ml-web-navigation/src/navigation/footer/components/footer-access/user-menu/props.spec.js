const footerAccessUserMenuProps = require('./props');
...
const myAccountFooterAccessItem = baseProps.configSchema.footerAccess.items.find(item => item.id === 'myAccount');

describe('Mercado Libre Navigation', () => {
  describe('Footer Access User Menu props', () => {
    it('returns props needed for the Footer Access User Menu Component', () => {
      const props = footerAccessUserMenuProps.props(baseProps, myAccountFooterAccessItem);

      expect(props).toBeDefined();
      expect(Array.isArray(props)).toBeDefined();
    });

    it('returns the sections when the user is logged in', () => {
      const sections = footerAccessUserMenuProps.getSectionForUser(true, myAccountFooterAccessItem);

      expect(sections).toHaveLength(3);
      expect(sections).toEqual(['myAccount', 'bookmarks', 'syi']);
    });

    it('returns the sections when the user is not logged in', () => {
      const sections = footerAccessUserMenuProps.getSectionForUser(
        false,
        myAccountFooterAccessItem,
      );

      expect(sections).toHaveLength(2);
      expect(sections).toEqual(['login', 'syi']);
    });
  });
});

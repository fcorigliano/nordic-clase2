...
const downloadAppBannerProps = require('./props');
...
jest.doMock('ml-navigation-config', () => ({
...
describe('Mercado Libre Navigation', () => {
  describe('Download App Banner props', () => {
    it('returns props needed for the Download App Banner Component', () => {
      const props = downloadAppBannerProps(baseProps);

      expect(props.label).toEqual(baseProps.navigationUrls.downloadAppStore.name);
      expect(props.link).toEqual('https://play.google.com/store/apps/details?id=com.mercadolibre');
      expect(props.asButton).toEqual(false);
      expect(props.target).toEqual(undefined);
    });

    it('returns props needed for the Download App Banner Component, retrieves link from download app link since device is not defined', () => {
      const newProps = { ...baseProps, device: { mobile: true, desktop: false, lowEnd: false, osName: 'not defined' } };
      const props = downloadAppBannerProps(newProps);

      expect(props.label).toEqual(baseProps.navigationUrls.downloadAppStore.name);
      expect(props.link).toEqual(baseProps.navigationUrls.downloadAppLanding.link);
      expect(props.asButton).toEqual(false);
      expect(props.target).toEqual(undefined);
    });

    it('returns props needed for the Download App Banner for desktop', () => {
      const desktopProps = { ...baseProps,
        device: {
          desktop: true,
        } };
      const props = downloadAppBannerProps(desktopProps);

      expect(props.label).toEqual(baseProps.navigationUrls.downloadAppLanding.name);
      expect(props.link).toEqual(baseProps.navigationUrls.downloadAppLanding.link);
      expect(props.asButton).toEqual(true);

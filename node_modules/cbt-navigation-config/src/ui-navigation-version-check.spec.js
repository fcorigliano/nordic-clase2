...
const uiNavigationModuleName = 'cbt-web-navigation';

const getUiNavigationVersionsToCheck = (config) => {
  const versions = [];
  if (((config.configurations) || {}).uiNavigationVersion) {
    versions.push({
      scope: 'global',
      version: config.configurations.uiNavigationVersion,
    });
  }
  versions.push(...Object.keys(config.platforms || {}).reduce(
    (platformVersions, platform) => {
      if (
        (config.platforms[platform].configurations || {}).uiNavigationVersion
      ) {
      ...
      const sitesUiVersions = Object.keys(config.platforms[platform].sites || {}).reduce((sitesVersion, site) => {
        if (config.platforms[platform].sites[site].uiNavigationVersion) {
          sitesVersion.push({

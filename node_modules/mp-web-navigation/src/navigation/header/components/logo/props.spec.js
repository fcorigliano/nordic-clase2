...
describe('Mercado Pago Navigation', () => {
  describe('Logo props', () => {
    it('returns props needed for the Logo Component', () => {
      const props = logoProps(baseProps);

      expect(props.url).toEqual(baseProps.urls.baseDomain.link);
      expect(props.label).toEqual(baseProps.labels.headTitle);
    });

    it('returns empty props since the type is hidden', () => {
      const props = logoProps({ ...baseProps, type: 'hidden' });
